                ...
                <el-collapse v-model="activeName" accordion>
                    <el-collapse-item :name="ticket.id" v-for="ticket in data" :key="ticket.id">
                        <template #title>
                            <div class="ticket-head">
                                <div class="ticket-title">
                                    ...
                                    <div>{{ ticket.name }}</div>
                                    <div>库存：<span>{{ ticket.sold_quantity }} / {{ ticket.total_quantity }}</span></div>
                                    <div>预留：<span>{{ ticket.reserved_quantity }}</span></div>
                                </div>

                                <div class="price">
                                    <div class="old_price" :class="{ 'discount': ticket.newPrice }">
                                        <span>{{ ticket.currency }} <strong>{{ ticket.oldPrice }}</strong>/人</span>
                                    </div>
                                    <div class="new_price" v-if="ticket.newPrice">
                                        <span>{{ ticket.currency }} <strong>{{ ticket.newPrice }}</strong>/人</span>
                                    </div>
                                </div>
                            </div>
                        </template>
                        ...
                        <div class="ticket-gird">
                            <div>
                                有效期：
                                <span v-if="ticket.validity_type === 1">购买后{{ ticket.validityDays }}天内有效</span>
                                <span v-else="ticket.validity_type === 2">{{ ticket.startDate }}至{{ ticket.endDate
                                }}</span>
                            </div>
                            <div>
                        ...
                        <div v-if="ticket.exchangeable">
                            改期规则：<div>{{ ticket.exchangeRules }}</div>
                        </div>
                        <div v-if="ticket.refundable">
                            退款规则：<div>{{ ticket.refundRules }}</div>
                        </div>
